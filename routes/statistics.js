...
router.get("/", auth, async (req, res) => {
  ...
  const { start: mostSoldStart, end: mostSoldEnd } = getDateRange(
    selectedMostSoldStockItemOption
  );
  const { start: newlyAddedStart, end: newlyAddedEnd } = getDateRange(
    selectedNewlyAddedUsersCountOption
  );
  ...
  const initializedOrdersCountPercent =
    totalOrders > 0
      ? ((initializedOrdersCount / totalOrders) * 100).toFixed(2)
      : 0;
  ...
  const ordersWithinPeriod = await Order.find({
    ...
    orderStatusId: {
      $in: [
        OrderStatusEnum.INITIALIZED,
        OrderStatusEnum.PROCESSING,
        OrderStatusEnum.DELIVERED,
      ],
    }, // Assuming 1 = initialized, 2 = processing, 3 = delivered
  });
  ...
  ordersWithinPeriod.forEach((order) => {
    order.items.forEach((item) => {
      ...
      if (stockItemCountMap.has(stockItemId)) {
        stockItemCountMap.set(
          stockItemId,
          stockItemCountMap.get(stockItemId) + amount
        );
      } else {
  ...
  if (mostSoldStockItemId) {
    const stockItem = await StockItem.findById(mostSoldStockItemId).select(
      "name"
    );
